...
export const getServicesTypesTemplatesStepId =
  "get-services-types-templates"
...
export const getServicesTypesTemplatesStep = createStep(
  ...
  async (
    ...
    StepResponse<GetServicesTypesTemplatesStepOutput>
  > => {
    const mpnBuilderService = container.resolve(
      MPN_BUILDER_MODULE
    ) as MpnBuilderService

    let listTemplateServices =
      mpnBuilderService.listTemplateServices()
    ...
    const newTemplates = await Promise.all(
      listTemplateServices.map(async (template: any) => {
