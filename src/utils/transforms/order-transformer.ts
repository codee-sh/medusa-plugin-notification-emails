import { formatDate, getFormattedAddress, getLocaleAmount, getTotalCaptured } from "../index"
...
export function transformOrderData(
  ...
  const transformedOrder = {
    ...
    transformed: {
      ...
      order_date: formatDate({ date: order.created_at, includeTime: true, localeCode }),
      ...(order.updated_at && {
        completedDate: formatDate({ date: order.updated_at, includeTime: true, localeCode }),
      }),
      total_amount: order.items?.reduce(
        (acc: number, item: any) => acc + (item.variant?.prices?.[0]?.amount || 0) * item.quantity,
        0
      ) || 0,
      ...
      summary: {
        total: getLocaleAmount(order.summary?.original_order_total || 0, order.currency_code),
        paid_total: getLocaleAmount(
          getTotalCaptured(order.payment_collections || []),
          order.currency_code
        ),
        tax_total: getLocaleAmount(order.tax_total || 0, order.currency_code),
        discount_total: getLocaleAmount(order.discount_total || 0, order.currency_code)
      },
    }
  }
